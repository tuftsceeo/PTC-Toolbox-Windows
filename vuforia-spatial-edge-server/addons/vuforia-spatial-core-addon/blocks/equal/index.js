...
exports.render = function (object, frame, node, block, index, thisBlock, callback) {
    // checks if values are equivalent - true output if first input is equal to second input
    if (thisBlock.data[0].value === thisBlock.data[1].value) thisBlock.processedData[0].value = 1;
    else thisBlock.processedData[0].value = 0;
